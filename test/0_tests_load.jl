...
@testset "load_files.jl" begin
    ...
    Replication_Carleton_et_al_2022.load_global_mortality_panel_covariates(pwd=string("../"))
    ...
    # Test the delete function of the same file:
    Replication_Carleton_et_al_2022.delete_Figure_1_estimates(pwd=string("../"))
    @test !isfile(path)

    # Test the load_covar_pop_count() function:
    Replication_Carleton_et_al_2022.load_covar_pop_count(pwd=string("../"))
    path = "../0_input/cleaned_data/covar_pop_count.dta"
    @test isfile(path)

    Replication_Carleton_et_al_2022.delete_covar_pop_count(pwd=string("../"))
    @test !isfile(path)
    ...
    # We set the variabel to nothing
    path = nothing
